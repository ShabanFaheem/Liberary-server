...
function getBookById(req, res) {
  ...
  pool.query("select * from book where book_id = $1", [id], (err, book) => {
    if (!err) {
      res.json(book.rows);
    }
  });
...
function deleteBook(req, res) {
  ...
  pool.query("DELETE FROM book where book_id = $1", [id], (err, book) => {
    if (!err) {
...
function borrowBook(req, res) {
  ...
  pool.query(
    "UPDATE book SET borrow_date=$1, return_date=$2, student_id=$3 WHERE book_id=$4;",
    [borrowDate, returnDate, studentId, id],
    (err, book) => {
      if (!err) {
        res.send("Book updated with id: " + id);
      } else {
...
function borrowedBooks(req, res) {
  const id = req.params.id;
  pool.query(
    "SELECT * from book where book.student_id = $1 ;"
  ,
    [id],
    function (err, result) {
      if (!err) {
